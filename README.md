...
## Introduction
In this POE we had to develop a web application using the react framework. It is meant to be a payment portal where a user can register and sign in, before making a payment and viewing a history of all their own payments. In the employee portal, the employee would be able to view all the users payments and review the details, to determine if the payment will be accepted or rejected. In the admin portal, the super admin would then manage the employee accounts where they can view, delete and add a new employee account. The portal should be able to hash and salt passwords and other sensitive data. The data in transit must be protected, where backend and frontend SSL certificates is needed. Regex patterns must be used for input fields to be sanitized and meet specific criteria. The user, admin and employee portals must be protected against a variety of attacks that include: NoSQL injection, session hijacking, DDOS, man in the middle, clickjacking, and cross site scripting attacks. We must also have a CircleCi pipeline configured and use sonarQube/sonarCloud to do a security scan on our application.
